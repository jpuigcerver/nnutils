...
void mask_image_from_size(
    const ConstTensor<IT>& sizes, MutableTensor<T>* batch,
    const typename MutableTensor<T>::DType& mask_value,
    const MaskImageCaller<typename MutableTensor<T>::DType>& caller) {
  assert(batch->Dims() == 4);
  assert(sizes.Dims() == 2);

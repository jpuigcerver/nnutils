...
void adaptive_maxpool_2d_fwd(
  ...
  // Resize the index tensor to have the proper size.
  if (index) { index->ResizeAs(*output); }
  ...
  auto out_idx_data = index ? index->Data() : nullptr;
  caller.Forward(
      N, C, inp_H, inp_W, out_H, out_W,
      inp_sizes_data, out_sizes_data,
...
void adaptive_maxpool_2d_bwd(
    const ConstTensor<IT>* output_sizes, const ConstTensor<IT>& index,
    const ConstTensor<T>& g_output, MutableTensor<T>* g_input,
    const AdaptiveMaxpool2dCaller<typename ConstTensor<T>::DType>& caller) {
  assert(g_output.Dims() == 4);
  assert(index.IsSameSizeAs(g_output));
  ...
  auto out_sizes_data = output_sizes ? output_sizes->Data() : nullptr;
  auto out_idx_data = index.Data();
  caller.Backward(
      N, C, inp_H, inp_W, out_H, out_W, out_sizes_data, out_idx_data, go_data,
      gi_data);
}
